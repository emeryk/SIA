...
void renderBlock(Scene* scene, ImageBlock& block)
{
    ...
    for (int y=0; y<size.y(); ++y) {
        for (int x=0; x<size.x(); ++x) {
            for (int n(0); n < camera->sampleCount(); n++)
            {
                double xp = x*1.0 + n*1.0/camera->sampleCount();
                for (int m(0); m < camera->sampleCount(); m++)
                {
                    double yp = y*1.0 + m*1.0/camera->sampleCount();
                    // compute the primary ray parameters
                    if (camera->isStratified())
                    {
                        float jitterX, jitterY;
                        jitterX = Eigen::internal::random(0,2);
                        jitterX -= 1;
                        jitterY = Eigen::internal::random(0,2);
                        jitterY -= 1;

                        Ray ray;
                        ray.origin = camera->position();
                        ray.direction = (camF + camX * (2.0*float(xp + jitterX / 100 + 0.5f + offset[0])/float(camera->vpWidth())  - 1.)
                                              - camY * (2.0*float(yp + jitterY / 100 + 0.5f + offset[1])/float(camera->vpHeight()) - 1.)).normalized();
                    ...
                    Ray ray;
                    ray.origin = camera->position();
                    ray.direction = (camF + camX * (2.0*float(xp + 0.5f + offset[0])/float(camera->vpWidth())  - 1.)
                                          - camY * (2.0*float(yp + 0.5f + offset[1])/float(camera->vpHeight()) - 1.)).normalized();

                    //Color3f moyenne += integrator->Li(scene, ray);
